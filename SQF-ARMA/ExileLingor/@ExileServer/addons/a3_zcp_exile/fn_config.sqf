...
ZCP_AI_useLaunchersChance = 5; // %Chance to spawn Launcher on AI soldier ( never exceeds the MIN and MAX defined per cappoint).

// Put the following to -1 to disable it. //default: 60
ZCP_AI_killAIAfterMissionCompletionTimer = 300; // Amount of seconds before all ZCP AI get auto killed after a mission is completed. ( DMS only ).
...
ZCP_MinWaitTime = 180; // seconds to wait to spawn a new capturepoint when 1 was capped.
ZCP_MaxWaitTime = 300; // random between 0 and THIS number added to the ZCP_MinWaitTime to counter spawning points at the same time //default: 120
ZCP_BaseCleanupDelay = 900; // seconds to wait to delete a captured base.

ZCP_RewardRelativeToPlayersOnline = true; // This will recalculate the crypto reward according the amount of online players.
ZCP_PoptabReward = 100; // Poptab reward for capping per player online. ( When poptab reward is selected or randomly chosen ).
ZCP_MinPoptabReward = 4000; // Poptabreward is added to this number

ZCP_ReputationReward = 20; // Respect reward for capping per  player online.
ZCP_MinReputationReward = 400; // ZCP_ReputationReward is added to this number
ZCP_ReputationRewardForGroup = 400; // Each group members gets this amount of reputation ( for the trouble).
ZCP_CONFIG_GroupDistanceForRespect = 300; // meters to be close to the capper to get the group award
...
/* 3.1 new configs */
ZCP_CONFIG_TerritoryDistance = 400;  // Distance from territories. ( 0 to disable )
...
ZCP_MissionMarkerWinDotTime = 300; // Seconds to show a marker after a capped point. Change to 0 to disable!
