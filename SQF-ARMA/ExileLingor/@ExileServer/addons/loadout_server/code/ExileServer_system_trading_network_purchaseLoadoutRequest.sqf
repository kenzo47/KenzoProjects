...
private ["_sessionID", "_parameters", "_purchaseLoadout", "_currentLoadout", "_playerObject", "_loadoutNumber", "_gearNumber", "_totalNumber", "_add", "_playerMoney", "_playerRespect", "_respectGain", "_logging", "_traderLog", "_responseCode", "_requiredRespect"];
...
try 
{
	...
	_requiredRespect = _purchaseLoadout call ExileClient_gui_LoadoutDialog_calculateLoadoutRespect;
	if (_playerRespect < _requiredRespect) then
	{
		throw 69; //do something with the error message
	};
	...
	_playerObject setUnitLoadout _purchaseLoadout; //yepcock
