...
EZM_ExtendedLogging					= false;			// Slightly more information in logs.
...
// Night settings
EZM_LightsOff						= false;             // Kill all the light on map except the player lights.           
EZM_NightStartTime					= 18;               // Time at which it is night in hours
EZM_NightEndTime					= 6;                // Time at which it is no longer night in hours

// Explosive zombies          
EZM_ExplosiveZombies				= false;			// randomly boobie trapped zombies exploding a few seconds after dying.
...
EZM_ZombieMoney						= 10;               // Money per zombie kill
EZM_ZombieMaxMoney					= 60;               // Max Money per zombie kill-random amount put on corpses

// Stats settings
EZM_EnableStatKill					= false;			// Enable stat tracking for Kills DB entry
EZM_EnableZombieStatKill			= true;				// Enable stat tracking for ZedKills DB entry
...
EZM_ZombieRespect					= 20;				// Respect per zombie kill

// Frag settings
EZM_RoadKillBonus					= 10;				// Bonus Respect if roadkill
EZM_MinDistance						= 20;				// Minimal distance for range bonus
EZM_CqbDistance						= 10;				// Minimal ditance for close quarter bonus
EZM_CqbBonus						= 20;				// Respect for close quarter bonus at 1 meter
...
EZM_ryanzombiesdamage						= 0.065;		// Attack damage *(% of players life per hit, 1 is 100%) //6.5% damage per hit
EZM_ryanzombiesdamagecar					= 0.025;		// Attack damage to car *(% of car health per hit, 1 is 100%) //2.5% damage per hit
...
EZM_UseHarassingZombies						= false;		// Spawn harassing zombies around the player.
...
/* 2 Loot group */           EZM_Trash,				// Loot function defined in ZLoot.sqf
/* 3 Zombie group */         EZM_Easy					// Group function defined in ZClasses.sqf
];

// Hordes Config
EZM_UseHorde								= true;		// Use the horde spawner  
EZM_HordeLoopTime							= 120;		// Spawn a Horde every X Minutes. (Default 30 mins)
EZM_HordeConfig = [
/* 0 Groups Size  */         10,						// maximum number of zombies around a player
/* 1 Vest group */           EZM_VeryBasic,					// Vest function defined in ZVest.sqf
/* 2 Loot group */           EZM_Decent,			// Loot function defined in ZLoot.sqf
/* 3 Zombie group */         EZM_MediumMix,				// Group function defined in ZClasses.sqf
/* 4 Horde density */        30							// Radius in which the zombies will spawn should be lower than Min Spawn Distance.
];
