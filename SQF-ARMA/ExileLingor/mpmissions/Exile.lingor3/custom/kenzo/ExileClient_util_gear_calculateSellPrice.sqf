...
else 
{
	...
	if (isNumber (missionConfigFile >> "CfgExileVehiclesSellPrice" >> _className >> "price")) then 
	{
		_amount = getNumber (missionConfigFile >> "CfgExileVehiclesSellPrice" >> _className >> "price");
		_amount = (_amount * 0.2);
		_amount = (floor _amount) max 1;
	};
};
