	...
	{
		{
			if (typeName _x isEqualTo "STRING") then
			{
				...
				{
					if (!(isClass (missionConfigFile >> "CfgLoadoutWhitelist" >> _itemClassName)) || ({_x == _itemClassName} count (getArray (MissionConfigFile >> "CfgLoadout" >> "Settings" >> "BlockedItems")) > 0)) then
					{
						_playerLoadout set [0,[]];
