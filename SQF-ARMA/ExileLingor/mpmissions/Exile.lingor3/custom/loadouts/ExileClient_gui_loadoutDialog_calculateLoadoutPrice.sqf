...
{
	{
		if (typeName _x isEqualTo "ARRAY") then
		{
			...
			{	
				...
				{
					_price = _price + getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> (_x select 0) >> "price");
				};
			};
		}
		else
		{
			if !(_x isEqualTo "") then
			{
				...
				{
					_price = _price + getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> _x >> "price");
				};
			};
		};
			...
			{
				{
					switch (count _x) do
					{
							...
							{
								{
									if (typeName _x isEqualTo "ARRAY") then
									{
										if (count _x > 0) then
										{
											...
											{
												_price = _price + (getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> (_x select 0) >> "price") * _quantity);
											};
										};
									}
									else
									{
										if !(_x isEqualTo "") then
										{
											...
											{
												_price = _price + (getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> _x >> "price") * _quantity);
											};
										};
									};
								} forEach (_x select 0);
						...
						case 3:
						{
							...
							{
								_price = _price + (getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> (_x select 0) >> "price") * (_x select 1));
							};
						};
						default 
						{
...
if !(_loadoutHeadgear isEqualTo "") then
{
	...
	{
		_price = _price + getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> _loadoutHeadgear >> "price");
	};
};
...
if !(_loadoutFacewear isEqualTo "") then
{
	...
	{
		_price = _price + getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> _loadoutFacewear >> "price");
	};
};
...
if (count _loadoutBinocular > 0) then
{
	...
	{
		_price = _price + getNumber (missionConfigFile >> "CfgLoadoutWhitelist" >> (_loadoutBinocular select 0) >> "price");
	};
};
